...
gcloud run deploy dcsmart-analytics-api --project=$PROJECT --region=$REGION \
  ...
  --set-env-vars "PGHOST=/cloudsql/$INSTANCE,DCSMART_DB=postgres,ANALYTICS_DB=dcsmart_analytics,PGUSER_RO=analytics_ro,PGUSER_APP=analytics_app,BQ_PROJECT=$PROJECT,BQ_DATASET=dcsmart_analytics,ANALYTICS_ALLOWED_ROLES=super_admin;dcsmart,FRONTEND_ORIGIN=https://analisis.dcsmart.app,VERTEX_LOCATION=$REGION,AI_MODEL=gemini-2.5-flash,GOOGLE_CLIENT_ID=288069746644-9m0lq9tkh3lgcr2c7tkdb1ncltegbido.apps.googleusercontent.com" \
  --set-secrets "PGPASSWORD_RO=analytics-ro-password:latest,PGPASSWORD_APP=analytics-app-password:latest,ANALYTICS_JWT_SECRET=analytics-jwt-secret:latest,INTERNAL_SHARED_SECRET=internal-shared-secret:latest" \
